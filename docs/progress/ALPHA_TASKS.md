...
## 16. V2 第二轮端到端验证 · openai-compat + gpt-5.3-chat-latest（2026-05-28 完成）

用户配置员工走 `openai-compat` provider + 自定义 base URL（ai-ng-test.alchemytech.cc）+ `gpt-5.3-chat-latest` 模型，再跑 KYC 注册接口需求。本轮把 §15 修不到的 openai-compat 路径 + 弱 tool-use 模型场景压出来，**连发 3 个 P0 + 1 个 P1 调优，引擎才从"完全跑不通 openai-compat 业务"修到"LLM 真改代码 + mvn 验证"**。

### 16.1 LLM 配置陷阱（user-facing）

| # | 现象 | 根因 |
|---|---|---|
| ① | `400 invalid model ID` | `.env` 写 `GPT‑5.3‑Codex`，含 Unicode U+2011 非 ASCII 连字符；上游 API 不识别 |
| ② | `404 This is not a chat model, did you mean v1/completions?` | `gpt-5.3-codex` 是 completion 模型，**不支持 tool-use**；codex 系列（5.1/5.2/5.3-codex）全部不兼容；换 `gpt-5.3-chat-latest` 才走 chat completions |

### 16.2 引擎层发现的 P0 bug

**bug 4 — composer 字符串化 chat history，openai-compat 端 tool-use 协议失效**（commit `5a9ddaf`）
- 现象：gpt-5.3-chat-latest 第 1 轮成功调 Bash，但**第 2 轮起 22 轮 stop loop**，每轮输出 ~2k 字符"我看不到本地代码，请贴出来"
- 根因：composer.extractText 把 assistant tool_call 序列化为 `"→ tool_call: Bash({...})"` 字符串、tool_result 包成 `{role:'user', content:'[tool_result]\n...'}`。这**只对 Anthropic 凑效**（Claude 凭文本能推断）；OpenAI chat completions 协议要求 assistant 工具调用走 message-level `tool_calls` 字段、tool result 走独立 `role:'tool' + tool_call_id` message。所以 openai-compat 端 LLM 看不到自己真发过 tool_call → 困惑
- 修复：types.ts 引入 `LLMContentBlock = text | tool_call | tool_result` 协议无关 IR；anthropic.ts / openai-compat.ts 各自翻译；composer/prompt-minimal 输出结构化 block 保留 callId。+ 13 个回归测试覆盖两端协议翻译

**bug 5 — emit_deliverable 拦不住"零业务工具调用"**（commit `cd6ec32`）
- 现象：gpt-5.3-chat-latest 在 bug 4 修后 + `DEFAULT_RECENT=30` 调优后，**第 1 轮 LLM 调用就直接 emit_deliverable**，把 Java 代码贴在 contentText 里假装完成，0 个 Bash/Edit/Write 调用
- 根因：§15 修的 `detectLastToolFailure` 只能拦"上一个工具失败"，拦不住"压根没调过工具"
- 对照参考实现（探索 OpenClaw + hermes-agent 的防谎报策略）：
  - **OpenClaw**：根本没有 `emit_deliverable` 类工具，用 push-based 完成模型 + tool-result-guard + tool-loop-detection（20+ 重复拦）
  - **hermes-agent**：`kanban_complete` 严格前置（task ownership / 必填 summary / 幻觉 card 检测）+ `TOOL_USE_ENFORCEMENT_GUIDANCE` prompt（"Never end your turn with a promise of future action — execute it now"）+ `OPENAI_MODEL_EXECUTION_GUIDANCE`（针对 gpt/qwen/deepseek 自动注入）+ `tool_result_classification.py` 识别 `bytes_written / success==true`
- 修复（套 hermes "前置检查"思路）：`countBusinessToolCalls(repos, threadId)` 数 thread 里 type='tool_call' message（系统工具如 advance_step / emit_deliverable 不写 tool_call message，所以等价于"业务工具 + spawn_employee"次数）。emit_deliverable case 增加 "businessToolCalls === 0 → block + 写 system error" 守卫。子工单（parentRequirementId ≠ null）跳过守卫
- 测试调整：12 个旧 mock 简化测试加 `setup.primeBusinessTool()` helper 注入业务工具调用让守卫放行（这些测试本来测状态机/工具分发，不是真实业务流）

**bug 6 — openai-compat 协议下 orphan tool_result → 400 Invalid parameter**（commit `2b409f4`）
- 现象：KYC 第 7 轮 LLM 真改了 KycController.java 跑到 ~14 轮时，server 报 `400 Invalid parameter: messages with role 'tool' must be a response to a preceeding message with 'tool_calls'`
- 根因：composer.tailByThread(threadId, `DEFAULT_RECENT=30`) 切窗口时把对应 tool_call 切走，留下 orphan tool_result message；openai-compat appendOpenAIMessages 直接把它转 `role:'tool'` 发出去 → OpenAI 协议要求 `role:'tool'` 前面必须紧跟含 `tool_calls` 的 assistant message
- 修复：buildBody 维护 `knownToolCallIds` Set 追踪本轮已发出的 tool_call ID；appendOpenAIMessages 收到 tool_result 时若 callId 不在 set 里 → 改写成 user 文本 `[tool_result of <id>]\n<output>`，既保留上下文又不违反协议。Anthropic 不受影响（tool_use_id 允许孤立）

### 16.3 P1 调优（commit `2af37af`）

`DEFAULT_RECENT = 10 → 30`：每个工具往返 2 message，10 条 = 5 对工具历史；第 6 轮起 LLM 真失忆，**反复 cat 同一文件 8 次**（每次以为自己第一次读）→ 撞 budget_iterations。30 条 ≈ 15 对工具往返。同时加 "🚫 防反复探索不动手" prompt 段（每文件最多 cat/grep 3 次 / 看完关键文件下一步必须写 / 看到自己重复读 = 失忆信号立即动手）

### 16.4 LLM 行为前后对比（同 gpt-5.3-chat-latest + 同 KYC 需求）

| 阶段 | 决策模式 | tool_call 数 | workdir 改动 |
|---|---|---|---|
| 第 5 轮（修 P0 #4 前） | 22 轮纯文本 stop loop | 0 | 0 |
| 第 5 轮（修 P0 #4 后） | 40 轮 Bash | 40（**全只读** cat/grep/rg） | 0 |
| 第 6 轮（DEFAULT_RECENT 30） | 1 轮直接 emit | 0（谎报） | 0 |
| **第 7 轮**（修完 3 个 P0 + prompt + 守卫） | 多轮 Bash + python3 写文件 + mvn | 多个含写 | **真改了 KycController.java** |

第 7 轮 LLM 行为（真改代码 → mvn 验证 → 守卫拦谎报）：
- seq 0-15: 4 次 cat KycController / PgcUtils / doc/createUser.md + grep userRegister
- seq 16: 试 `applypatch` 失败（不是真工具，是 LLM 凭直觉调的）
- seq 18: **改用 `python3 -<<EOF` + `Path.read_text/write_text` 真改文件**（自我修正）
- seq 20/22/28: 3 次 mvn compile（cwd / -pl 参数试错，都 exitCode=-1 timeout）
- seq 30+: LLM 反复 emit_deliverable，被 `detectLastToolFailure` 拦（mvn 失败 → 不能交付，防谎报机制工作）
- `git diff` 最终：KycController.java line 56 try {} 里加了 `redirectUrl ≤ 256` 长度守卫（doc 规定的边界检查）

### 16.5 实战教训

1. **不同 provider 的 tool-use 协议差异是引擎层 P0**——composer 必须输出协议无关 IR，让 provider 各自翻译。Anthropic"宽松"（凭文本能推断）+ OpenAI"严格"（要 message-level 字段）双协议测试不能只覆盖一端。
2. **弱 tool-use 模型（gpt-5.x，qwen 系，deepseek）需要更激进的引擎防御**——hermes 风格的"前置检查 + 强制 prompt"是必要的。OpenClaw 路径（不给 LLM 完成工具）是更彻底的方案，可作 V3 候选。
3. **`DEFAULT_RECENT` 是隐形的能力上限**——10 条窗口让 LLM 失忆陷入循环。每个工具往返 2 条，要给至少 15-20 对工具历史。
4. **codex 系列模型不支持 tool-use**——`gpt-5.x-codex` 都是 v1/completions 端点，引擎需要 chat completions + tool_calls 字段。文档 / setup 提示要加。
5. **架构对照很重要**：当遇到"LLM 谎报"这种系统性问题，先看 OpenClaw / hermes / 类似 agent 框架怎么做——能省一半设计时间。
6. **OpenAI 协议比 Anthropic 严格得多**——`role:'tool'` 必须紧跟 `tool_calls`、`tool_call_id` 必须匹配；任何 chat history 切割 / 重排都要保持配对。Anthropic 协议宽容，孤立 `tool_use_id` 都允许。

### 16.6 V2 第二轮收尾

- **新增代码**：commit `5a9ddaf..2b409f4`（3 个 P0 引擎修复 + 1 个 P1 prompt/窗口调优 + 13 个回归测试）
- **测试**：308 → 320 (+12)，2 fail 都是 EmbeddingService 网络问题（与改动无关），typecheck 全过
- **业务验证**：LLM 第 7 轮真改了 KycController.java，引擎 6 层防御全工作（IR 协议 / 守卫拦零工具 / orphan tool_result / detectLastToolFailure / DEFAULT_RECENT 调优 / prompt 强约束）
- **当前剩余局限**（非引擎 bug，留 V3 / 业务侧）：
  - mvn timeout（200s 不够大型 Java 项目）→ 调 Bash 工具默认 timeout 或加 cwd 校验
  - LLM 误以为"实现"实际是"补强"（接口已存在）→ prompt 引导 + 需求描述更精确
  - V3 候选：借鉴 OpenClaw push-based 完成模型（去掉 emit_deliverable 工具）+ tool-loop-detection

V2 引擎在 openai-compat + 弱 tool-use 模型上跑通端到端的基础能力建立完成。
