...
技能是项目的**长期资产**，不是一次性产物——**目标：未来其它项目 / 业务 / 员工都能直接拿来用**：

- **每次工单触达技能 → 顺手补**：发现盲区补步骤、出错补反例、有更优解就更新 `prompt_template`；不允许"技能放着不动越来越脱节"
- **设计要去项目化**：写"在 Spring Boot 项目中..." OK；写"在 lskj/virtual_card_api 中..." NOT OK——具体路径 / 业务规则 / 单点配置归工单或项目知识库（`project_knowledge`），不是技能
- **新业务先查现有技能**（按「🛡 改动前必读 §5」同精神，本仓库的"友商"就是已沉淀技能）：能复用就复用、缺一块就增量补、不允许另起一个高度重叠的新技能
- **跨员工绑定**：一个技能 ≠ 一个员工独占；通用技能（`Bash 工具规范` / `Maven 编译验证` / `Git 工作流` 等）应绑给所有相关员工；同岗位多员工共享岗位技能
- **健康度自检**（按 milestone 过一遍）：
  - 僵尸技能（长期没被任何工单激活）→ 评估删 / 合并
  - 内容高度重叠的技能 → 合并
  - 已绑定但 `prompt_template` 早过时（语义和 V2/V3 对不上）→ 更新
- 配套北极星：技能越完善 → 工单 prompt 越短（Token↓）+ 员工首次成功率↑（准确率↑）+ 同类任务收敛更快（效率↑）
